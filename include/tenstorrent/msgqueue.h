...
/** @brief Submessage for enabling block-power telemetry tags
 * @details Payload is a single uint32_t:
 * - Value == 0: Disable SERDES / GDDR / VCOREM (/ VCORE) power telemetry updates
 * - Value == 1: Enable SERDES / GDDR / VCOREM (/ VCORE) power telemetry updates
 */
struct char_block_power_telem_submsg {
	/** @brief 0 to disable, 1 to enable */
	uint32_t enabled;
};
...
union characterisation_submsg_data {
	...
	/** @brief Enable/disable SERDES / GDDR / VCOREM power telemetry */
	struct char_block_power_telem_submsg block_power_telem;
